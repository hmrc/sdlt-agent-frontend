...
class RichJsValueSpec
  ...
  "set" - {
    ...
    "must return an error if RecursiveSearch use" in {
      ...
      value.set(path, Json.obj()) mustEqual JsError(
        "recursive search not supported"
      )
    }
    ...
    "must return an error if RecursiveSearch use as second element" in {
      ...
      val path =
        JsPath(List() :+ KeyPathNode("key") :+ RecursiveSearch("child"))
      value.set(path, Json.obj()) mustEqual JsError(
        "recursive search is not supported:level-2"
      )
    }
