...
class ConfirmAgentContactDetailsControllerSpec
  ...
  trait Fixture {
    ...
    val agentContactDetails: AgentContactDetails =
    ...
    def noOnwardRoute: Call =
      Call("GET", "/stamp-duty-land-tax-agent/manage-agents/check-answers")

    val userAnswersWithAgentContactDetails: UserAnswers = Some(
      emptyUserAnswers
    ...
    val userAnswersWithOutAgentContactDetails: UserAnswers =
      userAnswersWithAgentContactDetails
        .remove(AgentContactDetailsPage)
        .success
        .value

    def yesOnwardRoute: Call = Call(
      "GET",
      "/stamp-duty-land-tax-agent/manage-agents/enter-contact-details"
    )

    val journeyRecoveryRoute: String =
      controllers.routes.JourneyRecoveryController.onPageLoad().url

    val messagesApi: MessagesApi = application.injector.instanceOf[MessagesApi]
    implicit val messages: Messages = messagesApi.preferred(FakeRequest())

    lazy val confirmAgentContactDetailsRoute: String =
    ...
    val form: Form[ConfirmAgentContactDetails] =
  ...
  "ConfirmAgentContactDetailsController" - {
    ...
    "must redirect to the agent contact details page when valid YES option is submitted" in new Fixture {
      ...
      override val application: Application = applicationBuilder(userAnswers =
        Some(emptyUserAnswers.set(AgentNamePage, agentName).success.value)
      )
        .build()
    ...
    "must remove AgentContactDetailsPage and redirect to CheckYourAnswersController when NO is submitted after previously selecting YES " in new Fixture {
      ...
      when(mockService.getAgentName(any())).thenReturn(Right(agentName))
      when(
        mockService.removeAgentContactDetailsPageAndUpdateUserAnswers(
          any(),
          any()
        )(any())
      )
        .thenReturn(
          Future.successful(Right(userAnswersWithOutAgentContactDetails))
        )

      override val application: Application =
        applicationBuilder(Some(userAnswersWithAgentContactDetails))
      ...
      running(application) {
        ...
        val request = FakeRequest(POST, confirmAgentContactDetailsRoute)
          .withFormUrlEncodedBody(
            ("value", ConfirmAgentContactDetails.values.tail.head.toString)
          )
    ...
    "must redirect to journey recovery when fail to extract agentName" in new Fixture {
      val mockService: StampDutyLandTaxService = mock[StampDutyLandTaxService]
      when(mockService.getAgentName(any())) thenReturn Left(
        Error("Failed to extract agentName")
      )
