...
class CheckYourAnswersController @Inject() (
  ...
  def onPageLoad(agentReferenceNumber: Option[String]): Action[AnyContent] =
      ...
      implicit request =>
        ...
        (storedArn, agentReferenceNumber) match {
          case (Some(storedArn), Some(paramArn)) if storedArn == paramArn =>
            logError(
              s"[CheckYourAnswersController][onPageLoad] storedArn: ${storedArn}, paramArn: ${paramArn}"
            )
            Future.successful(
          ...
          case (_, Some(paramArn)) =>
            stampDutyLandTaxService.getAgentDetails(
              ...
              case Some(agentDetails) =>
                stampDutyLandTaxService
                  ...
                  .fold(
                    { error =>
                      logError(
                        s"[CheckYourAnswersController][onPageLoad] Failed to build UA: ${error.getMessage}"
                      )
                      Future.successful(
                        Redirect(
              ...
              case None =>
                logError(
                  s"[CheckYourAnswersController][onPageLoad]: Failed to retrieve details for agent with agentReferenceNumber: $paramArn"
                )
                Future.successful(
                  Redirect(
              ...
              logError(
                s"[CheckYourAnswersController][onPageLoad] Unexpected failure: ${ex.getMessage}"
              )
              Redirect(controllers.routes.SystemErrorController.onPageLoad())
            }
          case _ =>
            logError(
              s"[CheckYourAnswersController][onPageLoad] ReloadPage from existing data from session}"
            )
            validateUserAnswers(request.userAnswers).fold(
              noSessionDataPage => Future.successful(noSessionDataPage),
              summaryListRows =>
                Future.successful(Ok(view(summaryListRows, postAction)))
            )
        }

    }

  def onSubmit(agentReferenceNumber: Option[String]): Action[AnyContent] =
    (identify andThen getData andThen requireData andThen stornRequired).async {
      implicit request =>
        agentReferenceNumber match {
          case None =>
            request.userAnswers.data.asOpt[CreatePredefinedAgentRequest] match {
              case None =>
                logError(
                  "[CheckYourAnswersController][onSubmit] Failed to construct AgentDetailsRequest"
                )
                Future.successful(
                  Redirect(
              ...
              case Some(createPredefinedAgentRequest) =>
                ...
                (for {
                  _ <- stampDutyLandTaxService.submitAgentDetails(
                  ...
                  navigator
                    .nextPage(AgentOverviewPage, NormalMode, updatedAnswers)
                ).flashing(
                  "agentCreated" -> createPredefinedAgentRequest.agentName
                )).recover { case ex =>
                  logError(
                    s"[CheckYourAnswersController][onSubmit] Unexpected failure: ${ex.getMessage}"
                  )
                  Redirect(
                    controllers.routes.SystemErrorController.onPageLoad()
                  )
          ...
          case Some(arn) =>
            request.userAnswers.data.asOpt[UpdatePredefinedAgent] match {
              case None =>
                logError(
                  "[CheckYourAnswersController][onSubmit Update] Failed to construct UpdatePredefinedAgent"
                )
                Future.successful(
                  Redirect(
                    controllers.routes.SystemErrorController.onPageLoad()
              ...
              case Some(updatePredefinedAgent) =>
                val updated =
                  updatePredefinedAgent.copy(agentResourceReference = Some(arn))
                logError(
                  s"[CheckYourAnswersController][onSubmit] updatedAgent: ${updated}"
                )
                (for {
                  _ <- stampDutyLandTaxService.updateAgentDetails(updated)
                  updatedAnswers <- Future.fromTry(
                    request.userAnswers.set(StornPage, request.storn)
                  )
                  _ <- sessionRepository.set(updatedAnswers)
                } yield Redirect({
                  navigator
                  ...
                  .recover { case ex =>
                    logError(
                      s"[CheckYourAnswersController][onSubmit update] Unexpected failure: ${ex.getMessage}"
                    )
                    Redirect(
                      controllers.routes.SystemErrorController.onPageLoad()
                    )
